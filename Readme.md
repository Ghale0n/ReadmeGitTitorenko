...
# Выгрузка и загрузка информации в интернет  
  Для того чтобы залить файл в сеть используется команда *git push*, предварительно необходимо подвязать удалённый репозиторий к локальному и выбрать ветку по умолчанию.
  Для того чтобы выкачать изменения с удалённого (подключённого) репозитория, используется команда *git pull*.
